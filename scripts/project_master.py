...
for dir in ["App", "components", "pages"]:
    ...
    for cmp in os.listdir(dir_path):
        ...
        if os.path.isdir(cmp_path):
            ...
            index_path = os.path.join(cmp_path, "index.ts")
            style_path = os.path.join(cmp_path, f"{cmp}.style.scss")

            # if os.path.exists(style_path):
            #     new_name = os.path.join(cmp_path, f"{cmp}.style.scss")
            #     os.rename(style_path, new_name)
            ...
            if os.path.exists(index_path):
                ...
                with open(index_path, "w") as index_file:
                    if os.path.exists(style_path):
                        counter += 1
                        index_file.write(f'import "./{cmp}.style.scss";\n\n')
            ...
            for sub_cmp in os.listdir(cmp_path):
                ...
                if os.path.isdir(sub_cmp_path):
                    ...
                    index_path = os.path.join(sub_cmp_path, "index.ts")
                    style_path = os.path.join(
                        sub_cmp_path, f"{sub_cmp}.style.scss")
